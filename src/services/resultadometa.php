...
try{
	...
	$sqldesativameta = "UPDATE metas SET met_status = 0";
	$stmtfor = $conexao->prepare($sqldesativameta);
	$stmtfor->execute();
	//$sqlatualizameta = "UPDATE metas SET met_status = 1 WHERE met_periodo_ini::date='2017-10-31' ::date";
        $sqlatualizameta = "UPDATE metas SET met_status = 1 WHERE met_periodo_ini::date='".$data."' ::date";

	$stmtfor = $conexao->prepare($sqlatualizameta);
	$stmtfor->execute();
...
where pedidos.data ='".$data."'  and  codigofab in (".$fornecedoresacessorios.") group by  pedidos.codigovend,nome,
    ...
    from trocas inner join
         ...
         vendedores on numero = codigovend where  trocas.data ='".$data."'  and  codigofab in (".$fornecedoresacessorios.")  group by trocas.codigovend,nome,
...
full join
         vendedores on numero = codigovend 
where pedidos.data ='".$data."'  and  codigofab in (".$fornecedorrevista.") and codigofab not in (".$fornecedoresacessorios.") group by  pedidos.codigovend,nome) as q full join
    (select trocas.codigovend ||' - '||nome as nomven,
    sum(itrocas.qtde * prcvenda) as valor_total_trocas
    from trocas inner join
         itrocas on trocas.documento = itrocas.documento full join
         vendedores on numero = codigovend where  trocas.data ='".$data."'  and  codigofab not in (".$fornecedoresacessorios.") and codigofab in (".$fornecedorrevista.") group by trocas.codigovend,nome) as g on g.nomven = q.nomven group by coalesce(q.nomven,g.nomven) order by revistavalor_total desc)revista)revista on vennomven = revista.revistanomven left join
...
full join
         vendedores on numero = codigovend 
where pedidos.data ='".$data."'  and  codigofab in (".$fornecedorparalelo.") and codigofab not in (".$fornecedoresacessorios.") group by  pedidos.codigovend,nome) as q full join
    (select trocas.codigovend ||' - '||nome as nomven,
    sum(itrocas.qtde * prcvenda) as valor_total_trocas
    from trocas inner join
         itrocas on trocas.documento = itrocas.documento full join
         vendedores on numero = codigovend where  trocas.data ='".$data."'  and  codigofab not in (".$fornecedoresacessorios.") and codigofab in (".$fornecedorparalelo.") group by trocas.codigovend,nome) as g on g.nomven = q.nomven group by coalesce(q.nomven,g.nomven) order by paralelovalor_total desc)paralelo)paralelo on vennomven = paralelo.paralelonomven inner join
    (SELECT met_valrev, 
           ven_cod||' - '||nome as metnomven, met_valpar, met_valaces
  FROM metas inner join  vendedores on numero = ven_cod where met_status= 1 and date_part('month',met_periodo_ini) = date_part('month','".$data."'::date))metas on  metas.metnomven = vennomven;";
  //
 //echo $sqlsel;
	//console.log($sqlsel);
