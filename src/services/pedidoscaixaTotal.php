...
$conexao = getConn();
$sql = "
select valor_total, valor_total_digitado from (
select sum(( coalesce(q.valor_total_pedidos,0) - coalesce(g.valor_total_trocas,0))) as valor_total from ( 
    ...
    trocas.doctoclie) as g on g.doctoclie = q.doctoclie) as total full join
    (select sum(caipag_valor) as valor_total_digitado from erp_caixa_pagamento where caipag_data_caixa = ':dat') as c on 1=1;";
