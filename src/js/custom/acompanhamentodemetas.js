...
function verificaMeta(){
  $.each($('.linha'), function(){
    var pct = $(this).children('.porcentagem').html();
    var thisdif = $(this).children('.diferenca').html();
    console.log(thisdif);
	...
	if((parseInt(pct)) < 95){
		...
		$(this).children('.atingiumeta').html("Não")
      ...
      $.each(data.data, function(key, values){
        if(values.loja_num !== 99){
          ...
          if($("#dataInicio").val() !== "" && $("#dataFim").val() !== ""){
            periodo = $("#dataInicio").val()+' a '+$("#dataFim").val();
          }
...
//            '<td class="dinheiro"><span id="valorTotalMetaMensal">'+values.meta_mensal.formatMoney(2, ',', '.')+'</span></td>'+
          ...
          $('#valorTotalMetaMensal').html(values.meta_mensal.formatMoney(2, ',', '.'));
        }
      });
      $('#totais tbody').html(tabela);
      $g('#loading').modal('close');
    },
    error: function(data){
      console.log(data);
...
function carregarVendasPeriodo(data1, data2){
  ...
  $.ajax({
    ...
    success: function(data) {      
      ...
      $('#metaAtualMostrando').html('periódicas');
      $('#metas tbody').html(tabela);
